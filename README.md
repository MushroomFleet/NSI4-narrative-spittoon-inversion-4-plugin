...
Four CORE cognitive frameworks guide AI generation quality. These are copied verbatim (byte-for-byte via `cp`) during `/nsi4-start` to prevent LLM paraphrase distortion. `/nsi4-frameworks` verifies they are present and generates the project manifest.
...
All four CORE files are immutable after placement — hooks block Write and Edit operations on them. If a CORE file is missing, `/nsi4-frameworks` restores it from the plugin source via literal copy. Custom GhostWritingStyle files (installed via `--ghost`) are preserved and not overwritten during restoration.
...
Each page targets 800-1500 words, references all bucket files for consistency, and applies all four cognitive frameworks.
...
| Frameworks | User installs manually | `/nsi4-start` copies CORE files via `cp`; `/nsi4-frameworks` verifies and generates manifest. Custom ghost styles via `--ghost`. Hybrid two-layer style system (scene-level + sentence-level) |
